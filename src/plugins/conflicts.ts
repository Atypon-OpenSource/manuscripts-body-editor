...
import { ConflictMerge, createMerge, iterateConflicts } from '../lib/merge'
...
const createDropdownWidget = (
  ...
  if (inlineContents) {
    ...
    rejectContainer.className = 'conflict-option'
    rejectContainer.addEventListener('click', handleRevert)

    const label = document.createElement('div')
    label.className = 'conflict-option-label'
    label.textContent = 'Accept current version'
    rejectContainer.appendChild(label)

    // TODO: || Fragment.from?
    const node = document.createElement('div')
    node.className = 'conflict-option-contents'
    node.appendChild(serializer.serializeFragment(inlineContents))
    rejectContainer.appendChild(node)

    popper.appendChild(rejectContainer)
  }

  // Revert to local version
  if (contents) {
    const acceptContainer = document.createElement('div')
    acceptContainer.className = 'conflict-option'
    acceptContainer.addEventListener('click', handleAccept)

    const label = document.createElement('div')
    label.className = 'conflict-option-label'
    label.textContent = 'Revert to your version'
    acceptContainer.appendChild(label)

    const node = document.createElement('div')
    node.className = 'conflict-option-contents'
    node.appendChild(serializer.serializeFragment(contents))
    acceptContainer.appendChild(node)

    popper.appendChild(acceptContainer)
  }
...
const hashComponents = (
  conflicts: LocalConflicts,
  newState: ManuscriptEditorState
) => {
  ...
  return new Plugin<ManuscriptSchema>({
        ...
        if (conflicts) {
          ...
          return {
            ...
            decorations: createDecorations(
              conflicts,
              newState.doc,
              props.applyLocalStep,
              props.applyRemoteStep
            ),
    ...
    props: {
      decorations(state: ManuscriptEditorState) {
        return conflictsKey.getState(state).decorations
      },
    },
  })
}
