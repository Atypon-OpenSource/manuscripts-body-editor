...
describe('editor view', () => {
  ...
  test('loads without plugins', () => {
    ...
    const tableOfContentsSection = view.state.doc.child(4).child(0)
    expect(tableOfContentsSection.content.child(1).attrs.contents).toBe(
      '<div>Table of Contents</div>'
    )

    const emptySection = view.state.doc.child(4).child(1)
    expect(emptySection.childCount).toBe(1)

    const sectionWithEmptyParagraphs = view.state.doc.child(4).child(4)
    expect(sectionWithEmptyParagraphs.childCount).toBe(3)

    const sectionWithList = view.state.doc.child(4).child(7)
  ...
  test('loads with plugins', () => {
    ...
    const sectionWithFigure = view.state.doc.child(4).child(8)
    ...
    const sectionWithTable = view.state.doc.child(4).child(9)
