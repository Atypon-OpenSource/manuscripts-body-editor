...
export interface HydratedNodes {
  current: ManuscriptNode
  ancestor: ManuscriptNode
  local: ManuscriptNode
}
...
const getConflict = async (
  ...
  const filteredDocs = await collection.pouch
...
export const applyRemoteStep = (
  ...
  return (
    ...
    remainingConflict.local.contents = (newLocal as ParagraphElement).contents
