...
export const getSelectedTableAttr = (state: ManuscriptEditorState) => {
  ...
  console.log('getSelectedTableAttr: ' + tableEl)
...
const getNeighbouringCells = (
  ...
  const tableDepth = nearestAncestor(isNodeOfType('table_body'))($firstCell)
  if (!tableDepth) {
  ...
  const neighbourCell =  (direction: 'left' | 'right' | 'top' | 'bottom') => {
